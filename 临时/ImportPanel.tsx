interface ImportPanelProps {
  ...
  onDeleteSelected: () => void; onRedo: () => void; onReverse: () => void; onRotate: (rotation: 90 | 180 | 270) => void; onUndo: () => void; selectedCount: number; statusMessage: string
}
  ...
  return (
    <section className="panel stack">
      ...
      <div className="button-grid action-grid-three">
        ...
        <button className="secondary-button" disabled={selectedCount === 0 || isBusy} onClick={onDeleteSelected} type="button">删选中</button>
        <button className="secondary-button" disabled={frameCount === 0 || isBusy} onClick={() => onRotate(90)} type="button">转 90°</button>
        <button className="secondary-button" disabled={frameCount === 0 || isBusy} onClick={() => onRotate(180)} type="button">转 180°</button>
        <button className="secondary-button" disabled={frameCount === 0 || isBusy} onClick={() => onRotate(270)} type="button">转 270°</button>
      </div>
